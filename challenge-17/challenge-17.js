...
*/
  ...
  console.log(text.replace(/Manuel Marques de Sousa/, "Leandro A. Almeida"));
...
*/
  ...
  console.log(text.replace(/[D-Z][a-h]/gi, "0"));
  // ?
...
*/
  ...
  console.log(text.replace(/a/gi, "4")); // mais simples
...
*/
  ...
  console.log(
    text.replace(/(O Centauro de Luvas)/g, function(fullMatch) {
...
*/
  ...
  function getMonthNumber(month) {
    return monthsObj[month] || "insira entre 01 a 12";
  }
 // usando FOR
  var monthsArray = Object.keys(monthsObj);
  for (var i = 0;  i < monthsArray.length; i++) {
    console.log(
        "O mês de " + monthsArray[i] + " é representado pelo número "+ getMonthNumber(monthsArray[i])
    );
  }
 // usando FOR IN
  for (var i in monthsObj) {
    console.log(
        "O mês de " + i + " é representado pelo número "+ getMonthNumber(i)
    );
  }

  // usando a própria função de getMonthNumber
  function getMonthNumber2(month) {
    ...
    return monthsObj[month] || monthsObj;
  }
  
  console.log(
    "O mês de Janeiro é representado pelo número "+ getMonthNumber("janeiro")
  );
...
*/
  ...
  var regexDate = /(\d\d) de (ju[nl]ho) de (\d\d\d\d)/g;
...
*/
  console.log("\nReplace de datas:");
  function replaceDate(fullMatch, group1, group2, group3) {
      return group1 + "/" + getMonthNumber(group2) + "/" + group3 
  }
